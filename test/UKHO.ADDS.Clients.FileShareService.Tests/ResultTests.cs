...
namespace UKHO.ADDS.Clients.FileShareService.Tests
{
    ...
    public class ResultTests
    {
        ...
        public async Task WithStreamData_Success(HttpStatusCode statusCode)
        {
            ...
            var response = new HttpResponseMessage { StatusCode = statusCode, Content = new StreamContent(stream) };
        ...
        public async Task WithObjectData_Success(HttpStatusCode statusCode)
        {
            ...
            var response = new HttpResponseMessage { StatusCode = statusCode, Content = new StringContent(JsonCodec.Encode(content)) };
        ...
        {
            var response = new HttpResponseMessage { StatusCode = statusCode, Content = new StringContent("this content is ignored") };
        ...
        {
            var badRequestContent = new ErrorResponseModel { CorrelationId = "1234", Errors = { new Error { Source = "Tomato", Description = "Red sauce" }, new Error { Source = "Pepper", Description = "Pepper sauce" } } };

            var badRequest = new HttpResponseMessage { StatusCode = HttpStatusCode.BadRequest, Content = new StringContent(JsonCodec.Encode(badRequestContent)) };
        ...
        {
            var internalServerError = new HttpResponseMessage { StatusCode = HttpStatusCode.InternalServerError, Content = null };
        ...
        public async Task NonSuccess_UnparseableContent()
        {
            ...
            var badRequest = new HttpResponseMessage { StatusCode = HttpStatusCode.BadRequest, Content = new StringContent(errorContent) };
